...
void Salmon::onExchangeMessage(const CExchangeMessage& em)
{
  // This strategy does nothing with exchange messages.
}

void Salmon::onChessMessage(const CChessMessage& cm)
{
  ...
  /*  unsigned sellPrice = 45;
    if(b.askPrice(0)-1>sellPrice) sellPrice = b.askPrice(0)-1;

  unsigned buyPrice = 30;
    if(b.bidPrice(0)+1<buyPrice) buyPrice = b.bidPrice(0)+1;
    if(buyPrice<20) buyPrice = 37;
*/
  unsigned sellPrice = 38;
  unsigned buyPrice = 36;
...
unsigned Salmon::computeFairPrice() const
{
...
int Salmon::getScore(char piece) const
{
...
unsigned Salmon::getPrice(int score) const
{
  // Scale the score to a price between 1 and 99.
  return .37;
  double price = 0.30 + (score*0.05);
